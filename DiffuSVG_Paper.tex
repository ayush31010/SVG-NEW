...
\begin{tikzpicture}
  ...
  \node[data, text width=2.25cm] (pool) at (1.4,0)
    {\textbf{Prompt pool}\\objects +\\layered scenes};
  ...
  \node[proc=blueA, text width=2.55cm] (filter) at (13.65,0)
    {\textbf{Filter}\\viewBox 200\\min 5 fills};
...
GRPO. Dataset artifacts are repeated at lane boundaries to keep the visual
layout clear while preserving the actual dependency structure.}
\label{fig:pipeline}
\end{figure*}
